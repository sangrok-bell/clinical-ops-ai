...
const draftFor = (f: ValFinding) =>
  `[${f.subject}] ${f.layer} ${layerStyle[f.layer].name}: ${f.msg} — 원자료 대조 요망`;
    ...
    <div className="mt-2 rounded-lg border border-[#eeeef4] bg-canvas p-2">
      <div className="flex items-center gap-1.5">
        ...
        {!resolved && (
          <button
            ...
            title="쿼리 수정"
            className="text-icon hover:text-brand"
          >
        ...
        {!resolved && (
          <button
            ...
            title="쿼리 삭제"
            aria-label="쿼리 삭제"
            className="text-icon hover:text-danger"
          >
      ...
      {editing ? (
        <textarea
          ...
          onBlur={() => {
            onUpdate(draft.trim() || query.text);
            setEditing(false);
          }}
          onKeyDown={(e) => {
      ...
      {resolved && query.reason && (
        <p className="mt-1 text-[11px] text-icon">종결 사유: {query.reason}</p>
      )}
    </div>
  );
}
...
function FindingItem({
  ...
  return (
    <div className="rounded-xl border border-[#eeeef4] bg-surface p-3 [animation-fill-mode:both] animate-[fadeIn_260ms_ease-out]">
      <div className="flex flex-wrap items-center gap-1.5">
        ...
        <span
          className={cn(
            ...
            f.tier === "absurd" ? "text-danger" : "text-info",
          )}
        >
          {f.verdict}
        </span>
      ...
      {f.evidence && (
        <p className="mt-0.5 text-[11px] text-icon">근거: {f.evidence}</p>
      )}
      {expl && (
        <p className="mt-2 rounded-md bg-elevated p-2 text-[11px] leading-relaxed text-ink">
          <span className="font-semibold text-brand">AI 검토 </span>
          {expl}
        </p>
      )}
      {query && (
        <QueryText query={query} onUpdate={onUpdate} onRemove={onRemove} />
      )}
      <div className="mt-2 flex items-center gap-2">
        <button
          onClick={ask}
          disabled={loading || !!expl}
          className="inline-flex items-center gap-1 text-[11px] font-medium text-brand hover:underline disabled:text-icon disabled:no-underline"
        >
          {loading ? (
...
function ManualQueries({
  ...
  onUpdate: (key: string, patch: Partial<Query>) => void;
      ...
      {open && (
        <div className="mt-3 flex flex-col gap-2">
          <input
            ...
            placeholder="대상자 ID (선택, 예: S-02)"
            className="rounded-lg border border-[#dddde4] bg-canvas px-2.5 py-1.5 text-sm text-ink outline-none focus-visible:ring-2 focus-visible:ring-brand"
          />
          <textarea
            value={text}
            onChange={(e) => setText(e.target.value)}
            rows={2}
            placeholder="쿼리 내용 (예: V3 PSG 측정일이 방문창을 벗어남 — 사이트 확인 요망)"
            className="rounded-lg border border-[#dddde4] bg-canvas px-2.5 py-1.5 text-sm text-ink outline-none focus-visible:ring-2 focus-visible:ring-brand"
          />
          <Button
            size="sm"
        ...
        <div className="mt-3 flex flex-col gap-2">
          {manual.map((q) => (
            <QueryText
              key={q.key}
              query={q}
              onUpdate={(t) => onUpdate(q.key, { text: t })}
...
export function Step5Review({
  ...
  const blindspot = result.records.filter(
    (r) => r.layer !== "L1" && r.layer !== "L2",
  ).length;
      ...
      <div className="mx-auto flex max-w-5xl flex-col gap-6">
        ...
        <div className="flex flex-col items-start gap-3 rounded-2xl border border-[#eeeef4] bg-surface p-6">
          <div className="flex items-center gap-2 text-sm text-dim">
            ...
            <span>
              코호트 적재됨 — 대상자 {dataset.subjects.length}명 · 설문{" "}
              {dataset.assessments.length}회차 · 스마트워치{" "}
              {dataset.watch.length} · AE {dataset.ae.length}
              {liveCount > 0 && (
                <>
                  {" "}
                  · <b className="text-ink">③ 수집 {liveCount}건 포함</b>
                </>
              )}
            </span>
          </div>
          <Button variant="navy" onClick={() => setRan(true)}>
            <ScanLine className="size-4" /> 대량 검증 실행
          </Button>
        </div>
      </div>
    );
    ...
    <div className="mx-auto flex max-w-5xl flex-col gap-5">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-5">
        {[
          ...
          <div
            key={c.k}
            className="rounded-2xl border border-[#eeeef4] bg-surface p-3"
          >
            <p className="text-[11px] text-dim">{c.k}</p>
            <p className={cn("mt-0.5 text-2xl font-bold tabular-nums", c.tone)}>
              {c.v}
            </p>
          </div>
        ))}
      ...
      <p className="-mt-2 text-[11px] text-icon">
        <b className="text-info">표준 EDC 사각지대 {blindspot}건</b>
        (L3·L5·L6·안전) 검출 · <b className="text-[#4d6a0f]">
          무신호 통과 0
        </b>{" "}
        보장 — 애매한 건도 조용히 통과하지 않고 검토 큐로 분류됩니다.
        {liveCount > 0 && <> ③에서 수집된 {liveCount}건 포함.</>}
      </p>
            ...
            <div key={q.route} className="flex flex-col gap-2">
              ...
              {items.length === 0 ? (
                <p className="rounded-xl border border-dashed border-[#eeeef4] px-3 py-4 text-center text-[11px] text-icon">
                  {q.route === "검토필요_SDV"
                    ? "이번 회차에는 추가 검토가 필요한 통계·교차 신호가 없습니다."
                    : "해당 없음"}
                </p>
              ) : (
                items.map((f) => (
                  <FindingItem
                    ...
                    onUpdate={(t) => onUpdate(keyOf(f), { text: t })}
                    onRemove={() => onRemove(keyOf(f))}
                  />
                ))
              )}
            </div>
      ...
      <div className="flex items-start gap-2 rounded-2xl bg-elevated p-3 text-[11px] leading-relaxed text-dim">
        ...
        <span>
          엔진은 쿼리를 <b className="text-ink">제안</b>할 뿐 — 사람이 문구를{" "}
          <b className="text-ink">수정·삭제·추가</b>하고, ⑥에서{" "}
          <b className="text-ink">사유와 함께 종결</b>합니다. 숫자·발화는 결정적
          규칙, 처분은 사람.
        </span>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3 rounded-2xl border border-[#eeeef4] bg-surface p-4">
        <p className="text-sm text-dim">
          ...
          <b className="text-ink">
            {issued}/{queryable.length}
          </b>{" "}
          · 발행된 쿼리는 ⑥에서 사유와 함께 종결돼야 Lock 가능
        </p>
        <Button
          variant="brand"
          className="ml-auto"
          ...
          onClick={onNext}
        >
          다음: ⑥ DB Lock <ArrowRight className="size-4" />
        </Button>
      </div>
    </div>
  );
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <p className="max-w-2xl text-[15px] leading-relaxed text-dim">
        누적 수집된 데이터 전체를{" "}
        <b className="text-ink">검증 엔진(analyze_bulk)</b>으로 점검합니다. 입력
        시점에 못 잡는
        <b className="text-ink">
          {" "}
          소스간 교차검증·진정성·안전성(L3·L5·L6·안전)
        </b>
        이 여기서 작동하고, 결과는 라우팅 큐로 분류돼 쿼리로 처리됩니다. 숫자는
        결정적 규칙, AI는 설명만.
      </p>
    </div>
  );
